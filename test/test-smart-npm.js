...
describe('smartNpm', function() {
  ...
  var npmSubCmd = npmCmds[0]
  var noNpmSubCmd = 'install'
  var mirrorRegistry = '--registry=' + env.NPM_MIRROR_REGISTRY
  var officialRegistry = '--registry=' + env.NPM_OFFICIAL_REGISTRY
  ...
  xcontext('cli', function() {
    ...
    it('should uninstall package `x-path` successfully', function(done) {
      this.timeout(100000)
      run(['uninstall', 'x-path'], function(status) {
        status.should.eql(0)
        done()
      })
    })
