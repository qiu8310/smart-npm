 ...
 *
 * * <a name="npm">需要去 registry 上修改或添加信息的命令（此模块返回的就是此类子命令）</a>
 *
 *  - `acccess`: 修改 package 的访问权限
 *  - `adduser`: 注册用户
 *  - `login`: 登录
 *  - `version`: 版本控制，`cnpm` 用了 `debug` 模块，所以它会劫持此命令，因而放 `npm` 中
 ...
 *
 *
 * * <a name="cnpm">需要去 registry 上查询信息的命令</a>
 *
 *  _说明：`cnpm` 模块默认提供了 `web`, `search`, `check`, `doc`, `user`, `sync`
 *  六个子命令，这些命令作用感觉都不是很大，一般用不到，除了 `sync` 外，都可以用 `npm` 原生的替换_
 *
 *  - `web`: `cnpm` 提供，跳转到 `cnpm` 主页 —— [http://npm.taobao.org/](http://npm.taobao.org/)
 *  - `search`: `cnpm` 和 `npm` 都提供了，用 `cnpm` 的话是跳到对应的[搜索页](http://npm.taobao.org/browse/keyword/jquery)，
 *    用 `npm` 的话是在当前命令行中出现搜索结果
 ...
 *  - `help`: 输出帮助信息
 *
 * * <a name="no">不需要去 registry 上操作的命令，即一般操作本地资源</a>
 *
